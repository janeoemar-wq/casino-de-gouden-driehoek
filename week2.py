...
import sys
from main import salutation,name
...
if soort_code < minimale_leeftijd:
    if soort_code == 0:
        print("Je krijgt geen toegang. u staat op onze black list")
        sys.exit(0)
    elif soort_code == 1:
        print(f"Process finished with exit code 1. Je krijgt geen toegang, bent te jong.")
        sys.exit(1)
    elif soort_code == 2:
        print(f"Process finished with exit code 2. Je krijgt geen toegang, je hebt te weinig budget.")
        sys.exit(2)
    elif soort_code == 3:
        print(f"Process finished with exit code 3. Je krijgt geen toegang, ID is niet geldig.")
        sys.exit(3)
    else:
        print("Je krijgt toegang.")
...
round_number = 1           # verplicht volgens opdracht
...
while True:
    ...
    # Stoppen bij 0
    if choice == 0:
        print(f"Beste {salutation}, u heeft het spel gestopt. bedankt voor het spelen, tot de volgende keer")
